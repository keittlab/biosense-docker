...
One OSX, you may need to run `brew install coreutils` to get `sha256.sum` for the download script. Note that this may change your system behavior as it puts GNU versions of some commands in your path. The command to burn an SD card on OSX is `sudo diskutil umount /dev/<devicename>; sudo dd bs=1M if=<raspbian-image-file> of=/dev/<devicename> status=progress`.
